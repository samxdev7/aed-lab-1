...
// Endpoint base para el algoritmo de burbuja
const path: string = `${API_KEY}/metodos/burbuja`;
...
export const BubbleSortPanel: React.FC<BubbleSortPanelProps> = ({ onBack }) => {
  ...
  const handleSort = () => {
    ...
    if (parsedArray.length > limit) {
      showNotification('warning', 'Límite excedido', `El arreglo contiene más elementos (${parsedArray.length}) que el tamaño definido (${limit}).`);
      return;
    }
    ...
    fetchRequest<number[]>(path, { method: 'POST', body: payload })
      .then((res) => {
        setSortedArray(res);
        showNotification('success', 'Ordenamiento completado', 'El arreglo fue ordenado exitosamente con el método de Burbuja.');
      })
      .catch((err) => {
        console.error(err);
  ...
  return (
    <div className="min-h-screen bg-[#0a0d18] text-white flex flex-col justify-between p-8 font-sans">
      ...
      <PanelHeader
        ...
        title="Método de Ordenación Burbuja"
