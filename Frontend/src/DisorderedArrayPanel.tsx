...
export const DisorderedArrayPanel: React.FC<DisorderedArrayPanelProps> = ({
  ...
  const exercises = [
    { id: 1, title: 'Ejercicio 4', desc: 'Registro de Empleados' },
    { id: 2, title: 'Ejercicio 5', desc: 'Renta de Departamentos' },
    { id: 3, title: 'Ejercicio 6', desc: 'Registro de Vendedores' },
  ];
          ...
          >
            <div className="flex items-center gap-4">
              <div className="p-3 rounded-xl bg-sky-500/10 text-sky-400 group-hover:bg-sky-500 group-hover:text-white transition-colors duration-300">
                {item.id === 1 && <User className="w-6 h-6" />}
                {item.id === 2 && <Home className="w-6 h-6" />}
                {item.id === 3 && <ShoppingBag className="w-6 h-6" />}
              </div>
              <div>
